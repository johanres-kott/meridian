    ...
    <div>
      ...
      <div style={{ marginBottom: isMobile ? 12 : 20 }}>
        ...
        <div>
          <div style={{ fontSize: isMobile ? 17 : 22, fontWeight: 500, color: "var(--text)" }}>
            {name}
            <span style={{ fontSize: isMobile ? 11 : 13, color: "var(--text-secondary)", fontFamily: "var(--font-mono)", marginLeft: isMobile ? 6 : 10 }}>{symbol}</span>
          </div>
        </div>

        <div style={{ display: "flex", alignItems: "baseline", gap: isMobile ? 8 : 12, marginTop: isMobile ? 4 : 8 }}>
          <span style={{ fontSize: isMobile ? 22 : 28, fontWeight: 300, fontFamily: "var(--font-mono)" }}>
          ...
          {change != null && (
            <span style={{ fontSize: isMobile ? 12 : 14, fontFamily: "var(--font-mono)" }}>
              <Chg value={change} />
            </span>
          )}
        </div>
      ...
      {(high > 0 || low > 0) && (
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 10 : 24, marginTop: isMobile ? 12 : 20 }}>
          {high > 0 && (
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-card)", padding: isMobile ? "12px 14px" : "16px 20px", minWidth: isMobile ? undefined : 140 }}>
              <div style={{ fontSize: isMobile ? 10 : 11, color: "var(--text-secondary)", marginBottom: isMobile ? 4 : 6 }}>Dagens hogsta</div>
              <div style={{ fontSize: isMobile ? 14 : 16, fontWeight: 300, fontFamily: "var(--font-mono)" }}>
