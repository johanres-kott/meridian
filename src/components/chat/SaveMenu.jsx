...
export default function SaveMenu({ content, onSaveStrategy, onSaveTodo }) {
  ...
  if (saved) return <span style={{ fontSize: 10, color: "var(--pos)" }}>{saved === "todo" ? "✓ Tillagd som att-göra" : saved === "strategy" ? "✓ Strategi sparad" : "✓ Insikt sparad"}</span>;
  ...
  return (
    <div style={{ position: "relative", marginTop: 4 }}>
      ...
      {open && (
        <div style={{ position: "absolute", bottom: "100%", left: 0, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-card)", padding: "4px 0", zIndex: 100, minWidth: 180, marginBottom: 4 }}>
      ...
      {showStocks && (
        <div style={{ position: "absolute", bottom: "100%", left: 0, background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-card)", padding: "4px 0", zIndex: 100, minWidth: 200, maxHeight: 200, overflow: "auto", marginBottom: 4 }}>
          {stocks.map(s => (
